...
def make_grid(X, Y, Z):
    ...
    for x in range(X):
        for y in range(Y):
            for z in range(Z):
                grid[ line_X[x]+line_Y[y]+line_Z[z] ] = (x,y,z)
    return grid
